...
test('loads the UK explorer and the Wainwright logbook without errors', async ({
  ...
  await expect(page.getByLabel('Explore All peaks')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Find a hill' })).toBeVisible();
  await expect(page.getByLabel('Hill list')).toHaveValue('all');
  ...
  await expect(page.getByText('214 hills shown')).toBeVisible();
  await expect(
