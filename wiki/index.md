---
hide:
  - navigation
  - toc
---

<div class="munro-home">
  <section class="munro-hero" aria-labelledby="munro-hero-title">
    <img
      class="munro-hero__terrain"
      src="assets/images/munro.png"
      width="1672"
      height="941"
      alt="Watercolor mountain range with a stone path, mist, lakes, and distant ridges"
    />
    <a class="munro-hero__site-cta" href="https://welbournesecurity.com/">
      Back to main site
    </a>

    <div class="munro-hero__grid">
      <div class="munro-hero__copy">
        <p class="munro-hero__kicker">Local-first peak logbook</p>
        <h1 id="munro-hero-title">Project Munro</h1>
        <p class="munro-hero__summary">
          A bone-and-ink outdoor logbook for marking the hills you have climbed,
          keeping your record on your device, and exporting it when you need it.
        </p>
        <svg class="munro-fault" viewBox="0 0 640 38" aria-hidden="true">
          <path d="M2 19 L72 19 L91 9 L136 28 L180 18 L224 18 L245 30 L296 14 L341 23 L393 12 L432 21 L474 20 L509 30 L553 13 L638 19" />
        </svg>
        <ul class="munro-hero__facts">
          <li>Wainwrights MVP built</li>
          <li>No accounts required</li>
          <li>Web first; iPhone and Android next</li>
        </ul>
      </div>

      <figure class="munro-monolith">
        <img
          src="assets/images/munro-portrait.png"
          width="1122"
          height="1402"
          alt="Watercolor mountain ridge rising above a stone path"
        />
        <figcaption>
          <span>01</span>
          <span>Summit record / carved into the map</span>
        </figcaption>
      </figure>
    </div>
  </section>
</div>
