...
- `public/` — committed PWA icons and favicon, generated by
  `npm run data:icons`
- `src/` — React app; `src/domain/` stays pure and framework-free
- `src/data/` — the hill-list registry (`lists.ts`), generated peak data
  (Wainwrights, Munros, Corbetts, Grahams, Donalds, Ethels, Hewitts,
  Marilyns), Lake District boundary
...
- `src/pwa/` — production-only service worker registration for the
  installable PWA shell (`vite-plugin-pwa`, configured in `vite.config.ts`)
